...
public class UpdateLogInterceptor implements Interceptor {
	...
	public UpdateLogInterceptor() {
		hideList.add("kr.co.ydns.grip.admin.page.system.mapper.ComScrnRqrMapper");
		hideList.add("kr.co.ydns.grip.admin.page.mngdt.mapper.MngdtRflbDataInsertMapper");
		hideList.add("kr.co.ydns.grip.admin.page.mngdt.mapper.MngdtDataInsertMapper");
	}
		...
		if (showSql(id)) {
			try {
				...
				if (result.length() > 1048576) {
					log.debug(Marker.SQL, "[update] {}", formattedSQL);
				} else {
					log.debug(Marker.SQL, "[update] {}", result);
				}
			} catch (Throwable e) {
				e.printStackTrace();
